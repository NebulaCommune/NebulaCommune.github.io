...
1.  打开 GitHub Desktop （Github Desktop页面如下）![alt text](<Github Desktop页面.png>)
2.  点击 `File > Clone repository...`
3.  在弹出的窗口中，选择你 Fork 的仓库（如 `your-username/NebulaCommune.github.io`），这里以`awesom-online-source`仓库为例![alt text](克隆页面.png)
4.  点击`Choose`可以选择本地存储路径，点击 **Clone** 完成克隆 （克隆完成的页面如下，可以发现左上角已经变成`awesome-online-source`）![alt text](克隆完成的页面.png)
...
### 2. 使用 VS Code 编辑代码
1.  在 GitHub Desktop 中，点击 `Repository > Open in Visual Studio Code`，或直接在 VS Code 中打开项目文件夹![alt text](VScode页面.png)
2.  在 VS Code 中进行代码修改、文档完善或资源添加（这里我以README.md为例）
3.  保存所有修改（`Ctrl+S` / `Cmd+S`）（修改后的页面如下）![alt text](修改后的页面.png)
...
### 1. 查看修改
1.  回到 GitHub Desktop，左侧面板会显示所有修改的文件![alt text](提交界面.png)
